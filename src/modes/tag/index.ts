...
export const tagMode: Mode = {
  ...
  async prepare({
    ...
    const userAllowedMCPTools = parseAllowedTools(userClaudeArgs).filter(
      (tool) =>
        tool.startsWith("mcp__github_") || tool.startsWith("mcp__linear_"),
    );

    // Check if Linear is enabled
    const linearEnabled = !!context.inputs.linearApiKey;
    ...
    // Add Linear tools when Linear API key is provided
    if (linearEnabled) {
      tagModeTools.push(
        "mcp__linear__get_teams",
        "mcp__linear__get_workflow_states",
        "mcp__linear__get_labels",
    ...
    const ourMcpConfig = await prepareMcpConfig({
      ...
      mode: "tag",
      context,
      linearApiKey: context.inputs.linearApiKey,
      linearTeamId: context.inputs.linearTeamId,
    });
